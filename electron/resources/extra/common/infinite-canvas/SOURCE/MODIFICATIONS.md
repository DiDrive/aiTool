...
Uploaded video nodes now support non-destructive trim and an independent reverse-analysis range.
Double-clicking a video opens millisecond range controls and fast, standard, or detailed analysis
profiles. Reverse analysis respects the selected absolute source range, captures denser timeline
evidence, and packs timestamped frames into ordered contact sheets so long videos are no longer
represented by at most twenty isolated moments.

Reverse-engineered scripts and storyboards can retain absolute start/end times, stable character and
scene identifiers, evidence timestamps, confidence, and visual-style profiles. Script adaptation now
accepts an uploaded or pasted plot brief and offers structure-lock, rhythm-preserving, and story-first
modes while preserving the original reverse-engineered branch.
