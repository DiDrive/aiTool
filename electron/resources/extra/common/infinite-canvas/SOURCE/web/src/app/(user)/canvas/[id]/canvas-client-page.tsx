...
import { CanvasVideoTrimDialog } from "../components/canvas-video-trim-dialog";
...
import { consistencyResultPatch, ensureWorkflowGenerationContract, formatStoryboardShots, hasValidWorkflowStructure, inferConsistencyFromScenes, inferConsistencyFromShots, isConcreteVisualBible, workflowStructureRepairPrompt, workflowTextResultPatch } from "../utils/canvas-workflow-structure";
import {
    CanvasNodeType,
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const createTrimmedVideoNode = useCallback(async (sourceNode: CanvasNodeData, range: { startMs: number; endMs: number }) => {
        if (!sourceNode.metadata?.content) throw new Error("原视频不可用");
        const rawBridge = window.sessionStorage.getItem("aigcpanel-clip-bridge");
        const bridge = rawBridge ? JSON.parse(rawBridge) as { url?: string; token?: string } : null;
        if (!bridge?.url || !bridge.token) {
            message.error("当前运行环境未连接桌面裁剪服务，请重启工作台后重试");
            return;
        }
        const hideLoading = message.loading("正在调用 FFmpeg 生成裁剪视频…", 0);
        try {
            const sourceUrl = await resolveMediaUrl(sourceNode.metadata.storageKey, sourceNode.metadata.content);
            const sourceBlob = await downloadRemoteMedia(sourceUrl);
            const response = await fetch(`${bridge.url}/clip`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${bridge.token}`,
                    "Content-Type": sourceBlob.type || "video/mp4",
                    "X-Clip-Start-Ms": String(Math.round(range.startMs)),
                    "X-Clip-End-Ms": String(Math.round(range.endMs)),
                },
                body: sourceBlob,
            });
            if (!response.ok) {
                const payload = await response.json().catch(() => null) as { message?: string } | null;
                throw new Error(payload?.message || `裁剪服务返回 HTTP ${response.status}`);
            }
            const clippedBlob = await response.blob();
            const uploaded = await uploadMediaFile(clippedBlob, "video-clip");
            const size = fitNodeSize(uploaded.width || sourceNode.metadata.naturalWidth || 1280, uploaded.height || sourceNode.metadata.naturalHeight || 720, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
            const id = `video-clip-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
            const clipNode: CanvasNodeData = {
                id,
                type: CanvasNodeType.Video,
                title: `${sourceNode.title || "视频"} · 裁剪 ${formatClipRange(range.startMs, range.endMs)}`,
                position: { x: sourceNode.position.x + sourceNode.width + 80, y: sourceNode.position.y },
                width: size.width,
                height: size.height,
                metadata: { ...videoMetadata(uploaded), workflowSourceNodeId: sourceNode.id, videoTrim: { startMs: 0, endMs: range.endMs - range.startMs }, videoAnalysisRange: { startMs: 0, endMs: range.endMs - range.startMs }, videoAnalysisMode: "detailed" },
            };
            nodesRef.current = [...nodesRef.current, clipNode];
            connectionsRef.current = [...connectionsRef.current, { id: nanoid(), fromNodeId: sourceNode.id, toNodeId: id }];
            setNodes(nodesRef.current);
            setConnections(connectionsRef.current);
            setSelectedNodeIds(new Set([id]));
            setSelectedConnectionId(null);
            setVideoEditorNodeId(null);
            message.success("已生成新的裁剪视频节点，原视频保持不变");
        } catch (error) {
            message.error(error instanceof Error ? error.message : "生成裁剪视频失败");
        } finally {
            hideLoading();
        }
    }, [message]);
    ...
    const handleCreateStoryboardScript = useCallback((scriptNode: CanvasNodeData) => {
        ...
        const node = createCanvasNode(preset.type, { x: scriptNode.position.x + scriptNode.width + 120, y: scriptNode.position.y }, { ...preset.metadata, workflowSourceNodeId: scriptNode.id, characterProfiles: scriptNode.metadata?.characterProfiles, sceneProfiles: scriptNode.metadata?.sceneProfiles, visualBible: scriptNode.metadata?.visualBible });
    ...
    const handleAdaptWorkflowScript = useCallback((scriptNode: CanvasNodeData, requirement: string, mode: "structure" | "rhythm" | "story") => {
        ...
        const modeContract = mode === "structure"
    ...
    const handleGenerateWorkflowAssets = useCallback((storyboardNode: CanvasNodeData, shotIds: string[]) => {
        const consistency = resolveWorkflowConsistency(storyboardNode, nodesRef.current);
        const shots = storyboardNode.metadata?.storyboardShots || [];
        ...
        const ensureAsset = (kind: "character" | "scene" | "prop", name: string, shot: CanvasStoryboardShot) => {
            ...
            const preset = workflowNodePreset(kind);
            const prompt = buildWorkflowAssetPrompt(preset.metadata.prompt || "", kind, name, shot, consistency);
            if (existingId) {
            ...
            const node = createCanvasNode(CanvasNodeType.Image, { x: storyboardNode.position.x + storyboardNode.width + 140 + (created.length % 3) * (spec.width + 36), y: storyboardNode.position.y + Math.floor(created.length / 3) * (spec.height + 70) }, { ...preset.metadata, prompt, excludeUpstreamText: true, workflowSourceNodeId: storyboardNode.id, workflowShotId: shot.id, workflowAssetKind: kind, workflowAssetName: name });
        ...
        const nextNodes = [...nodesRef.current, ...created].map((node) => promptByNodeId.has(node.id)
    ...
    const handleAnalyzeWorkflowConsistency = useCallback(async (workflowNode: CanvasNodeData) => {
        const sourceVideo = findWorkflowSourceVideo(workflowNode, nodesRef.current, connectionsRef.current);
        if (!sourceVideo) {
            message.warning("没有找到该剧本对应的原视频节点，无法从关键帧分析具体画风");
            return false;
        }
        const generationConfig = buildGenerationConfig(effectiveConfig, workflowNode, "text");
        if (!isAiConfigReady(generationConfig, generationConfig.model)) {
            openConfigDialog(true);
            return false;
        }
        ...
        try {
            const prompt = [
                "你是影视美术指导与角色设定师。请只根据上游视频关键帧，输出可直接约束图片/视频生成模型的具体一致性档案。",
                "禁止使用‘依据原视频’、‘保持原风格’、‘未指定’、‘待分析’等占位话术。必须明确判断媒介类型和地域审美，例如欧美3D动画、日式二维动画、真人电影等；真人与3D动画绝不能混淆。",
                "visualBible 必须具体描述：medium（媒介/渲染类型）、visualStyle（地域与造型风格）、colorPalette（具体主色与对比关系）、lightingStyle（具体照明和渲染）、cameraLanguage、editingRhythm、aspectRatio。",
                "characters 必须逐人描述可见年龄、性别呈现、脸型、肤色、发型、体型、服装颜色材质和3D/2D造型特征；locations 必须描述建筑/陈设、空间布局、材质和光线。无法确认的局部可以写‘画面证据不足’，但不能把整个字段写成泛化占位词。",
                '只输出 JSON：{"characters":[{"id":"character_1","name":"人物","appearance":"具体外貌与造型","wardrobe":"具体服装","voice":"可见/可听声音特征","firstSeenMs":0}],"locations":[{"id":"scene_1","name":"场景","environment":"具体环境与材质","layout":"具体空间结构","lighting":"具体光线","firstSeenMs":0}],"visualBible":{"medium":"具体媒介","visualStyle":"具体画风","colorPalette":"具体配色","lightingStyle":"具体照明渲染","cameraLanguage":"具体镜头语言","editingRhythm":"具体剪辑节奏","aspectRatio":"具体画幅"}}',
            ].join("\n\n");
            const analysisContext = await hydrateNodeGenerationContext(buildNodeGenerationContext(workflowNode.id, nodesRef.current, [...connectionsRef.current, { id: "style-analysis-source", fromNodeId: sourceVideo.id, toNodeId: workflowNode.id }], prompt), { analyzeVideos: true });
            const answer = await requestImageQuestion(generationConfig, buildNodeChatMessages(analysisContext), () => undefined);
            const patch = consistencyResultPatch(answer);
            if (!patch?.visualBible || !isConcreteVisualBible(patch.visualBible)) throw new Error("模型没有返回足够具体的画风档案，请重试");
            handleWorkflowMetadataChange(workflowNode.id, patch);
            message.success("已根据视频关键帧更新具体画风、人物和场景设定");
            return true;
        } catch (error) {
            message.error(error instanceof Error ? error.message : "视频风格分析失败");
            return false;
        } finally {
            setRunningNodeId(null);
        }
    }, [effectiveConfig, handleWorkflowMetadataChange, message, openConfigDialog]);
    ...
    const handleGenerateWorkflowStoryboards = useCallback((storyboardNode: CanvasNodeData, shotIds: string[]) => {
        ...
        const consistency = resolveWorkflowConsistency(currentStoryboard, nodesRef.current);
        const targetSet = new Set(shotIds);
        const created: CanvasNodeData[] = [];
        const runIds = new Set<string>();
        ...
        const nextShots = shots.map((shot, index) => {
            ...
            const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
            const prompt = `${preset.metadata.prompt || ""}\n镜头 ${shot.shotNumber}，${shot.shotSize}，${shot.durationSeconds} 秒。\n场景：${shot.scene}\n人物：${shot.characters.join("、")}\n道具：${shot.props.join("、")}\n额外参考：${(shot.extraAssets || []).map((asset) => `${asset.name}（${asset.kind}）`).join("、") || "无"}\n画面：${shot.visual}\n动作：${shot.action}\n运镜与光线：${shot.camera}；${shot.lighting}\n连续性：${shot.continuity}\n\n${formatShotConsistency(shot, consistency)}`;
            const node = createCanvasNode(CanvasNodeType.Image, { x: storyboardNode.position.x + storyboardNode.width + 140 + (index % 3) * (spec.width + 36), y: storyboardNode.position.y + 520 + Math.floor(index / 3) * (spec.height + 70) }, { ...preset.metadata, prompt, workflowSourceNodeId: storyboardNode.id, workflowShotId: shot.id, workflowAssetKind: "storyboard", workflowAssetName: `镜头 ${shot.shotNumber}` });
    ...
    const handleGenerateWorkflowVideos = useCallback((storyboardNode: CanvasNodeData, shotIds: string[]) => {
        ...
        const nextShots = shots.map((shot, index) => {
            ...
            const nextShot = index < shots.length - 1 ? shots[index + 1] : undefined;
            const sharesNextBoundary = Boolean(nextShot && sameContinuousScene(shot, nextShot) && nextShot.assetNodeIds?.storyboard);
            const firstFrameNodeId = shot.assetNodeIds?.storyboard;
            const lastFrameNodeId = sharesNextBoundary ? nextShot?.assetNodeIds?.storyboard : undefined;
            const prompt = `${preset.metadata.prompt || ""}\n镜头 ${shot.shotNumber}，时长 ${shot.durationSeconds} 秒。\n画面与动作：${shot.visual}；${shot.action}\n额外参考：${(shot.extraAssets || []).map((asset) => `${asset.name}（${asset.kind}）`).join("、") || "无"}\n运镜：${shot.camera}\n对白与声音：${shot.dialogue}；${shot.sound}\n连续性：${shot.continuity}\n${formatAdjacentShotContinuity(previousShot, shot, nextShot, sharesNextBoundary)}\n\n${formatShotConsistency(shot, consistency)}`;
        ...
        nodesRef.current = [...nodesRef.current, ...created].map((node) => node.id === storyboardNode.id
            ...
            : promptByNodeId.has(node.id)
                ? (() => {
                    const shotIndex = nextShots.findIndex((shot) => shot.assetNodeIds?.video === node.id);
                    const shot = shotIndex >= 0 ? nextShots[shotIndex] : undefined;
                    const nextShot = shotIndex >= 0 ? nextShots[shotIndex + 1] : undefined;
                    return { ...node, metadata: { ...node.metadata, prompt: promptByNodeId.get(node.id), excludeUpstreamText: true, firstFrameNodeId: shot?.assetNodeIds?.storyboard, lastFrameNodeId: shot && nextShot && sameContinuousScene(shot, nextShot) ? nextShot.assetNodeIds?.storyboard : undefined } };
                })()
                : node);
        connectionsRef.current = synchronizeWorkflowShotConnections(storyboardNode.id, shots, nextShots, connectionsRef.current, nodesRef.current);
        setNodes(nodesRef.current);
    ...
    const executeCanvasAgentAction = useCallback(
        async (action: CanvasAgentAction, messageReferenceNodeIds: string[]): Promise<CanvasAgentToolResult> => {
            ...
            const isAgentWriteAction = action.name === "generate_image" || action.name === "edit_image" || action.name === "generate_video" || action.name === "generate_audio" || action.name === "create_text_node" || action.name === "update_text_node" || action.name === "update_node" || action.name === "set_video_analysis_range" || action.name === "reverse_video_to_script" || action.name === "delete_node" || action.name === "create_connection" || action.name === "delete_connection" || action.name === "create_group" || action.name === "arrange_nodes";
            ...
            try {
                ...
                if (action.name === "set_video_analysis_range") {
                    const nodeId = stringValue("nodeId");
                    const node = getNode(nodeId);
                    if (!node) return missingNodeResult(nodeId);
                    if (node.type !== CanvasNodeType.Video || !node.metadata?.content) return { ok: false, code: "invalid_video_node", message: "只能设置已有内容的视频节点" };
                    const durationMs = Math.max(1, Number(node.metadata.durationMs || 0));
                    const trimStartMs = typeof args.trimStartMs === "number" ? Math.round(args.trimStartMs) : node.metadata.videoTrim?.startMs ?? 0;
                    const trimEndMs = typeof args.trimEndMs === "number" ? Math.round(args.trimEndMs) : node.metadata.videoTrim?.endMs ?? durationMs;
                    const analysisStartMs = Math.round(Number(args.analysisStartMs));
                    const analysisEndMs = Math.round(Number(args.analysisEndMs));
                    if (trimStartMs < 0 || trimEndMs <= trimStartMs || trimEndMs > durationMs) return { ok: false, code: "invalid_trim_range", message: `裁剪范围必须位于 0-${durationMs}ms` };
                    if (analysisStartMs < 0 || analysisEndMs <= analysisStartMs || analysisEndMs > durationMs) return { ok: false, code: "invalid_analysis_range", message: `分析范围必须位于 0-${durationMs}ms` };
                    const analysisMode = "detailed" as const;
                    const nextNodes = nodesRef.current.map((item) => item.id === nodeId ? { ...item, metadata: { ...item.metadata, videoTrim: { startMs: trimStartMs, endMs: trimEndMs }, videoAnalysisRange: { startMs: analysisStartMs, endMs: analysisEndMs }, videoAnalysisMode: analysisMode } } : item);
                    commitNodes(nextNodes);
                    return { ok: true, nodeId, videoTrim: { startMs: trimStartMs, endMs: trimEndMs }, videoAnalysisRange: { startMs: analysisStartMs, endMs: analysisEndMs }, videoAnalysisMode: analysisMode };
                }

                if (action.name === "reverse_video_to_script") {
                    const videoNodeId = stringValue("videoNodeId");
                    const videoNode = getNode(videoNodeId);
                    if (!videoNode) return missingNodeResult(videoNodeId);
                    if (videoNode.type !== CanvasNodeType.Video || !videoNode.metadata?.content) return { ok: false, code: "invalid_video_node", message: "倒推剧本需要一个已上传或已生成的视频节点" };
                    const preset = workflowNodePreset("script");
                    const nodeCenter = nextNodeCenter(CanvasNodeType.Text, [videoNode]);
                    const requirement = stringValue("requirement");
                    const prompt = `${preset.metadata.prompt || ""}${requirement ? `\n\n用户对本次倒推的补充要求：${requirement}` : ""}`;
                    const scriptNode = createCanvasNode(CanvasNodeType.Text, nodeCenter, { ...preset.metadata, sourceVideoNodeId: videoNodeId, prompt, status: NODE_STATUS_LOADING });
                    scriptNode.title = stringValue("title") || `倒推剧本 · ${videoNode.title}`;
                    const connection = { id: nanoid(), fromNodeId: videoNodeId, toNodeId: scriptNode.id };
                    commitNodes([...nodesRef.current, scriptNode]);
                    commitConnections([...connectionsRef.current, connection]);
                    selectOnly(scriptNode.id);
                    await handleGenerateNode(scriptNode.id, "text", prompt);
                    const generated = getNode(scriptNode.id) || scriptNode;
                    if (generated.metadata?.status === NODE_STATUS_ERROR) return { ok: false, code: "video_reverse_failed", message: generated.metadata.errorDetails || "视频倒推失败", nodeId: scriptNode.id };
                    return { ok: true, nodeId: scriptNode.id, connectionId: connection.id, node: canvasAgentNodeSummary(generated) };
                }
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                >
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            onOpenVideoEditor={(node) => setVideoEditorNodeId(node.id)}
                ...
                <CanvasWorkflowDetailModal
                    ...
                    onAnalyzeConsistency={handleAnalyzeWorkflowConsistency}
                ...
                <CanvasVideoTrimDialog
                    node={videoEditorNodeId ? nodes.find((node) => node.id === videoEditorNodeId) || null : null}
                    open={Boolean(videoEditorNodeId)}
                    onClose={() => setVideoEditorNodeId(null)}
                    onConfirm={(patch) => {
                        if (videoEditorNodeId) handleWorkflowMetadataChange(videoEditorNodeId, patch);
                        setVideoEditorNodeId(null);
                        message.success("已保存视频裁剪与分析范围");
                    }}
                    onCreateClip={createTrimmedVideoNode}
                />
...
function synchronizeWorkflowShotConnections(storyboardNodeId: string, previousShots: CanvasStoryboardShot[], nextShots: CanvasStoryboardShot[], connections: CanvasConnection[], nodes: CanvasNodeData[]) {
    ...
    shotIds.forEach((shotId) => {
        ...
        const nextShotIndex = next ? nextShots.findIndex((shot) => shot.id === next.id) : -1;
        const followingShot = nextShotIndex >= 0 ? nextShots[nextShotIndex + 1] : undefined;
        const boundaryFrameId = next && followingShot && sameContinuousScene(next, followingShot) ? followingShot.assetNodeIds?.storyboard : undefined;
        ...
        const managedVideoSources = [...managedAssets, previousStoryboardId || "", nextStoryboardId || "", boundaryFrameId || ""];
        registerTarget(previousVideoId, managedVideoSources, previousVideoId === nextVideoId ? [storyboardNodeId, ...nextAssets, nextStoryboardId || "", boundaryFrameId || ""] : []);
        registerTarget(nextVideoId, managedVideoSources, [storyboardNodeId, ...nextAssets, nextStoryboardId || "", boundaryFrameId || ""]);
    });
...
function sameContinuousScene(current: CanvasStoryboardShot, next: CanvasStoryboardShot) {
    const sameScene = Boolean(current.sceneId && next.sceneId ? current.sceneId === next.sceneId : current.scene.trim() && current.scene.trim() === next.scene.trim());
    const currentCharacters = new Set(current.characterIds?.length ? current.characterIds : current.characters.map((name) => name.trim().toLocaleLowerCase()));
    const nextCharacters = next.characterIds?.length ? next.characterIds : next.characters.map((name) => name.trim().toLocaleLowerCase());
    const sharesCharacter = !currentCharacters.size || !nextCharacters.length || nextCharacters.some((name) => currentCharacters.has(name));
    return sameScene && sharesCharacter;
}

function formatAdjacentShotContinuity(previous: CanvasStoryboardShot | undefined, current: CanvasStoryboardShot, next: CanvasStoryboardShot | undefined, sharesNextBoundary: boolean) {
    const lines = ["【相邻片段衔接约束】"];
    if (previous) lines.push(`承接镜头 ${previous.shotNumber} 的结束状态：${previous.action || previous.visual}。保持人物朝向、站位、服装、道具持握、环境布局、光线方向与色温连续。`);
    lines.push(`本镜头开始状态：${current.visual}；${current.action}。动作必须从上一状态自然延续，避免人物、道具或背景瞬移。`);
    if (next && sharesNextBoundary) lines.push(`本镜头结尾必须准确到达镜头 ${next.shotNumber} 的开场构图：${next.visual}；${next.action}。当前尾帧与下一片段首帧使用同一张分镜图。`);
    else if (next) lines.push(`下一镜头切换到场景“${next.scene}”，本镜头保持自身场景稳定，不提前变形或混入下一场景元素。`);
    return lines.join("\n");
}
...
function findWorkflowSourceVideo(node: CanvasNodeData, nodes: CanvasNodeData[], connections: CanvasConnection[]) {
    const queue = [node.id, node.metadata?.sourceVideoNodeId, node.metadata?.workflowSourceNodeId].filter(Boolean) as string[];
    ...
    while (queue.length) {
        ...
        if (current?.type === CanvasNodeType.Video && current.metadata?.content) return current;
        if (current?.metadata?.sourceVideoNodeId) queue.push(current.metadata.sourceVideoNodeId);
        if (current?.metadata?.workflowSourceNodeId) queue.push(current.metadata.workflowSourceNodeId);
        connections.filter((connection) => connection.toNodeId === nodeId).forEach((connection) => queue.push(connection.fromNodeId));
    }
    return undefined;
}

function resolveWorkflowConsistency(node: CanvasNodeData, nodes: CanvasNodeData[]) {
    let current: CanvasNodeData | undefined = node;
    const visited = new Set<string>();
    while (current && !visited.has(current.id)) {
        visited.add(current.id);
        if (current.metadata?.characterProfiles?.length || current.metadata?.sceneProfiles?.length || current.metadata?.visualBible) {
            return { characters: current.metadata.characterProfiles || [], scenes: current.metadata.sceneProfiles || [], visualBible: current.metadata.visualBible };
        }
        current = current.metadata?.workflowSourceNodeId ? nodes.find((item) => item.id === current?.metadata?.workflowSourceNodeId) : undefined;
    }
    const inferred = node.metadata?.workflowKind === "storyboard_script" ? inferConsistencyFromShots(node.metadata.storyboardShots || []) : inferConsistencyFromScenes(node.metadata?.scriptScenes || []);
    return { characters: inferred.characterProfiles, scenes: inferred.sceneProfiles, visualBible: inferred.visualBible };
}

function buildWorkflowAssetPrompt(
    presetPrompt: string,
    kind: "character" | "scene" | "prop",
    name: string,
    shot: CanvasStoryboardShot,
    consistency: ReturnType<typeof resolveWorkflowConsistency>,
) {
    return [
        presetPrompt,
        `素材名称：${name}`,
        "只生成上述单个素材，不要加入其他人物、场景、道具、剧情动作、对白、分镜构图或镜头事件。",
        formatAssetConsistency(kind, name, shot, consistency),
    ].filter(Boolean).join("\n\n");
}

function formatAssetConsistency(kind: "character" | "scene" | "prop", name: string, shot: CanvasStoryboardShot, consistency: ReturnType<typeof resolveWorkflowConsistency>) {
    const sections = [formatAssetVisualBiblePrompt(consistency.visualBible)];
    if (kind === "character") {
        const characterIndex = shot.characters.findIndex((item) => item === name);
        const profileId = characterIndex >= 0 ? shot.characterIds?.[characterIndex] : undefined;
        const profile = consistency.characters.find((item) => item.name === name)
            || (profileId ? consistency.characters.find((item) => item.id === profileId) : undefined);
        sections.push(profile ? `【人物一致性设定】\n姓名：${profile.name}\n外貌：${profile.appearance || "未指定"}\n服装：${profile.wardrobe || "未指定"}` : `【人物一致性设定】\n人物：${name}。严格沿用剧本中的年龄、面部、发型、体型和服装信息。`);
    }
    if (kind === "scene") {
        const profile = consistency.scenes.find((item) => item.name === name || item.id === shot.sceneId);
        sections.push(profile ? `【场景一致性设定】\n场景：${profile.name}\n环境：${profile.environment || "未指定"}\n空间结构：${profile.layout || "未指定"}\n光线：${profile.lighting || "未指定"}` : `【场景一致性设定】\n场景：${name}。严格沿用剧本中的时代、环境、空间结构和光线。`);
    }
    if (kind === "prop") sections.push(`【道具一致性设定】\n道具：${name}。只呈现该道具，固定其造型、材质、颜色和尺度；不要加入使用者或使用情境。`);
    return sections.filter(Boolean).join("\n\n");
}

function formatAssetVisualBiblePrompt(bible: CanvasNodeMetadata["visualBible"]) {
    if (!bible) return "";
    return `【素材视觉规范】\n媒介：${bible.medium || "未指定"}\n整体画风：${bible.visualStyle || "未指定"}\n色彩：${bible.colorPalette || "未指定"}\n光线与渲染：${bible.lightingStyle || "未指定"}\n画幅：${bible.aspectRatio || "未指定"}`;
}

function formatShotConsistency(shot: CanvasStoryboardShot, consistency: ReturnType<typeof resolveWorkflowConsistency>) {
    const characters = consistency.characters.filter((item) => shot.characters.includes(item.name) || shot.characterIds?.includes(item.id));
    const scene = consistency.scenes.find((item) => item.name === shot.scene || item.id === shot.sceneId);
    return [
        formatVisualBiblePrompt(consistency.visualBible),
        characters.length ? `【本镜头人物一致性】\n${characters.map((item) => `${item.name}：外貌 ${item.appearance || "未指定"}；服装 ${item.wardrobe || "未指定"}；声音 ${item.voice || "未指定"}`).join("\n")}` : "",
        scene ? `【本镜头场景一致性】\n${scene.name}：环境 ${scene.environment || "未指定"}；空间 ${scene.layout || "未指定"}；光线 ${scene.lighting || "未指定"}` : "",
    ].filter(Boolean).join("\n\n");
}

function formatVisualBiblePrompt(bible: CanvasNodeMetadata["visualBible"]) {
    if (!bible) return "";
    return `【视觉总规范】\n媒介：${bible.medium || "未指定"}\n整体画风：${bible.visualStyle || "未指定"}\n色彩：${bible.colorPalette || "未指定"}\n光线风格：${bible.lightingStyle || "未指定"}\n镜头语言：${bible.cameraLanguage || "未指定"}\n剪辑节奏：${bible.editingRhythm || "未指定"}\n画幅：${bible.aspectRatio || "未指定"}`;
}
...
function formatClipRange(startMs: number, endMs: number) {
    return `${(Math.max(0, startMs) / 1000).toFixed(3)}-${(Math.max(startMs, endMs) / 1000).toFixed(3)}秒`;
}
