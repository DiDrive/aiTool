...
import { CanvasNodeType, type CanvasAgentState, type CanvasConnection, type CanvasNodeData, type CanvasScriptScene, type CanvasStoryboardShot, type CanvasVideoAnalysisMode, type CanvasVideoRange, type CanvasWorkflowNodeKind } from "../types";
...
function summarizeNode(node: CanvasNodeData): CanvasAgentContextNode {
    ...
    return {
        ...
        workflowKind: node.metadata?.workflowKind,
        scriptScenes: node.metadata?.scriptScenes,
        storyboardShots: node.metadata?.storyboardShots,
        videoTrim: node.metadata?.videoTrim,
        videoAnalysisRange: node.metadata?.videoAnalysisRange,
        videoAnalysisMode: node.metadata?.videoAnalysisMode,
    };
}
