...
export const CANVAS_AGENT_ACTION_NAMES = [
    ...
    "update_text_node",
    "update_node",
    "set_video_analysis_range",
    "reverse_video_to_script",
    "delete_node",
    "create_connection",
    "delete_connection",
    "create_group",
...
const WORKFLOW_ASSET_KIND = { type: "string", enum: ["character", "scene", "prop", "storyboard"] };
...
export const CANVAS_AGENT_TOOLS: CanvasAgentToolDefinition[] = [
    ...
    defineTool("set_video_analysis_range", "设置视频节点的倒推分析区间。时间均为原视频绝对毫秒，系统固定使用精细分析。兼容可选的非破坏裁剪范围，但分析范围与裁剪范围相互独立。", { nodeId: STRING, trimStartMs: { type: "number", minimum: 0 }, trimEndMs: { type: "number", minimum: 1 }, analysisStartMs: { type: "number", minimum: 0 }, analysisEndMs: { type: "number", minimum: 1 } }, ["nodeId", "analysisStartMs", "analysisEndMs"]),
    defineTool("reverse_video_to_script", "真实分析视频关键帧并创建中文结构化倒推剧本。可先用 set_video_analysis_range 指定区间。", { videoNodeId: STRING, title: STRING, requirement: STRING }, ["videoNodeId"]),
    ...
    defineTool(
        ...
        "调用现有图片任务链路。sourceNodeIds 只放真实直接来源，独立生成必须传空数组；其中图片按数组顺序编号为图片1、图片2。",
        {
            ...
            workflowAssetKind: WORKFLOW_ASSET_KIND,
    ...
    defineTool(
        ...
        { prompt: STRING, title: STRING, sourceNodeIds: STRING_ARRAY, size: STRING, count: { type: "number", minimum: 1, maximum: 15 }, workflowShotId: STRING, workflowAssetKind: WORKFLOW_ASSET_KIND, workflowAssetName: STRING },
        ["prompt", "sourceNodeIds"],
    ),
    defineTool(
        "generate_video",
...
export function normalizeCanvasAgentAction(name: unknown, args: unknown, id = nanoid()): CanvasAgentAction {
    ...
    switch (actionName) {
        ...
        case "update_node":
            ...
            break;
        case "set_video_analysis_range": {
            normalized = {
                nodeId: requiredString(input.nodeId, "nodeId"),
                ...(boundedNumber(input.trimStartMs, 0, Number.MAX_SAFE_INTEGER) !== undefined ? { trimStartMs: boundedNumber(input.trimStartMs, 0, Number.MAX_SAFE_INTEGER) } : {}),
                ...(boundedNumber(input.trimEndMs, 1, Number.MAX_SAFE_INTEGER) !== undefined ? { trimEndMs: boundedNumber(input.trimEndMs, 1, Number.MAX_SAFE_INTEGER) } : {}),
                analysisStartMs: boundedNumber(input.analysisStartMs, 0, Number.MAX_SAFE_INTEGER),
                analysisEndMs: boundedNumber(input.analysisEndMs, 1, Number.MAX_SAFE_INTEGER),
            };
            if (normalized.analysisStartMs === undefined || normalized.analysisEndMs === undefined || Number(normalized.analysisEndMs) <= Number(normalized.analysisStartMs)) throw new Error("分析结束时间必须晚于开始时间");
            break;
        }
        case "reverse_video_to_script":
            normalized = { videoNodeId: requiredString(input.videoNodeId, "videoNodeId"), ...(optionalString(input.title) ? { title: optionalString(input.title) } : {}), ...(optionalString(input.requirement) ? { requirement: optionalString(input.requirement) } : {}) };
            break;
        ...
        case "edit_image": {
            ...
            normalized = {
                ...
                ...(optionalString(input.workflowShotId) ? { workflowShotId: optionalString(input.workflowShotId) } : {}),
                ...(optionalString(input.workflowAssetKind) ? { workflowAssetKind: optionalString(input.workflowAssetKind) } : {}),
                ...(optionalString(input.workflowAssetName) ? { workflowAssetName: optionalString(input.workflowAssetName) } : {}),
            };
...
export function canvasAgentActionLabel(action: CanvasAgentAction) {
    const labels: Record<CanvasAgentActionName, string> = {
        ...
        update_node: "正在更新节点",
        set_video_analysis_range: "正在设置视频分析范围",
        reverse_video_to_script: "正在分析视频并倒推剧本",
        delete_node: "正在删除节点",
...
export function userLikelyRequestedCanvasAction(text: string) {
    return /(?:创建|新增|插入|修改|更新|删除|连接|连线|分组|整理|生成|生图|视频|音频|配音|旁白|执行|拆成|拆分|放到画布|开始做|倒推|分析|裁剪)/i.test(text);
}
