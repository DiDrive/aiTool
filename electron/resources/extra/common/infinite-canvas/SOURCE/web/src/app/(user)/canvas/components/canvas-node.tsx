...
type CanvasNodeProps = {
    ...
    onOpenVideoEditor?: (node: CanvasNodeData) => void;
...
function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    return <TrimmedVideo node={node} />;
}

function TrimmedVideo({ node }: { node: CanvasNodeData }) {
    const ref = useRef<HTMLVideoElement>(null);
    const trim = node.metadata?.videoTrim;
    return (
        <div className="relative h-full w-full">
            <video
                ...
                onLoadedMetadata={(event) => {
                    if (trim?.startMs) event.currentTarget.currentTime = trim.startMs / 1000;
                }}
                onTimeUpdate={(event) => {
                    if (!trim || event.currentTarget.currentTime * 1000 < trim.endMs) return;
                    event.currentTarget.pause();
                    event.currentTarget.currentTime = trim.startMs / 1000;
                }}
            />
            {trim ? <span className="pointer-events-none absolute bottom-12 left-2 rounded bg-black/65 px-2 py-1 text-[10px] text-white">裁剪 {formatVideoRange(trim.startMs, trim.endMs)}</span> : null}
        </div>
    );
}

function formatVideoRange(startMs: number, endMs: number) {
    const compact = (ms: number) => {
        const seconds = Math.max(0, ms) / 1000;
        return `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${(seconds % 60).toFixed(1).padStart(4, "0")}`;
    };
    return `${compact(startMs)}–${compact(endMs)}`;
}
