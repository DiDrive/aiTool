...
export async function hydrateNodeGenerationContext(context: NodeGenerationContext, options: { analyzeVideos?: boolean } = {}) {
    ...
    const analysisContract = [
        "【视频倒推要求】",
        `本次只分析原视频绝对时间 ${formatAnalysisMs(videoAnalysis.rangeStartMs)} 至 ${formatAnalysisMs(videoAnalysis.rangeEndMs)}。系统共提取 ${videoAnalysis.evidenceFrameCount} 张证据帧，并按时间片整理成 ${videoAnalysis.frames.length} 张联系表：${videoAnalysis.timeline.join("、")}。`,
...
function formatAnalysisMs(ms: number) {
...
function readReferenceVideo(node: CanvasNodeData): ReferenceVideo | null {
    ...
    return {
        ...
        trimStartMs: node.metadata.videoTrim?.startMs,
        trimEndMs: node.metadata.videoTrim?.endMs,
        analysisStartMs: node.metadata.videoAnalysisRange?.startMs,
        analysisEndMs: node.metadata.videoAnalysisRange?.endMs,
        analysisMode: node.metadata.videoAnalysisMode,
    };
}
